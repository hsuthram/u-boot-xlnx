...
#define CONFIG_EXTRA_ENV_SETTINGS \
	...
	"loadbootenv=load mmc 1:$partid ${loadbootenv_addr} ${bootenv}\0" \ 
	"sd_uEnvtxt_existence_test=test -e mmc 1:$partid /uEnv.txt\0" \ 
	"uenvboot=" \ 
	...
	"sd_update_boot=echo Updating boot from SD; mmcinfo && fatload mmc 1:1 ${clobstart} ${boot_img} && run install_boot\0" \ 
	"install_boot=sf probe 0 && sf erase ${bootstart} ${bootsize} && " \ 
		"sf write ${clobstart} ${bootstart} ${filesize}\0" \ 
	"install_bootenv=mmcinfo && fatwrite mmc 1 ${clobstart} ${bootenv_img} ${filesize}\0" \ 
